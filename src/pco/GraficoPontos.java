...
public class GraficoPontos implements OutputTextual {
	 ...
	 */
	@Override
	public String outputTexto(Quantificavel q) { //PERGUNTAR!!!!!
		this.textos = new String[q.valores().size()];
		this.valores = new double[q.valores().size()];
		int iterar = 0;
		...
		for (Map.Entry<String,Double> entry : q.valores().entrySet()) {
			this.textos[iterar] = entry.getKey();
			this.valores[iterar] = entry.getValue();
			iterar++;
		}
	 ...
	 */
	@Override
	public String outputTexto(List<Par> lp) { //PERGUNTAR!!!!!
		// Usar o mesmo raciocinio aplicado no metodo anterior
		
		this.textos = new String [lp.size()];
		this.valores = new double [lp.size()];
		
		for (int i = 0; i < lp.size(); i++) {
			this.textos[i] = lp.get(i).primeiro();
			this.valores[i] = lp.get(i).segundo();
		}
	...
	public boolean ehRepresentavel(Quantificavel q) { 
		if (q != null && q.valores().size() > MAXIMO_LINHAS) {
			return true;
		}
		return false;
	}
	...
	public boolean ehRepresentavel(List<Par> lp) {
        if (lp != null && lp.size() > 0 && lp.size() < MAXIMO_LINHAS) {
        	return true;
        }
        return false;
	}
